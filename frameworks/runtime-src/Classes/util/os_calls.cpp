...
extern "C"
{
    ...
    PE_CORE_DECL NPL::NPLReturnCode NPL_activate_script_helloworld_cpp(NPL::INPLRuntimeState* pState);
}
...
#else
    static std::map<std::string, NPL::NPL_C_Func_ActivationFile::NPL_Activate_CallbackFunc> funcMap =
    {
        std::map<std::string, NPL::NPL_C_Func_ActivationFile::NPL_Activate_CallbackFunc>::value_type("NPL_activate_protocol_pb_cpp", NPL_activate_protocol_pb_cpp),
        std::map<std::string, NPL::NPL_C_Func_ActivationFile::NPL_Activate_CallbackFunc>::value_type("NPL_activate_script_helloworld_cpp", NPL_activate_script_helloworld_cpp)
    };
    
    
    auto it = funcMap.find(Fnname);
    if (it == funcMap.end())
