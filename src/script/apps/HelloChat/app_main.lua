...
use the lib:
------------------------------------------------------------
NPL.load("(gl)script/apps/HelloChat/app_main.lua");
------------------------------------------------------------
]]
-- create class
commonlib.setfield("MyCompany.HelloChat", {});
...
function MyCompany.HelloChat.OnConnection(app, connectMode)
	if(connectMode == System.App.ConnectMode.UI_Setup) then
		...
		-- e.g. Create a HelloChat command link in the main menu 
		local commandName = "Profile.HelloChat.Login";
		...
		if(command == nil) then
			...
			commandName = "Profile.HelloChat.Register";
			command = Map3DSystem.App.Commands.AddNamedCommand(
				{name = commandName, app_key = app.app_key, ButtonText = "注册新用户",  icon = app.icon, });			
				
			commandName = "Profile.HelloChat.HomePage";
			command = System.App.Commands.AddNamedCommand(
				{name = commandName,app_key = app.app_key, ButtonText = "HelloChat Front Page", icon = app.icon, });	
				
			commandName = "Profile.HelloChat.Rooms";
			command = System.App.Commands.AddNamedCommand(
				{name = commandName,app_key = app.app_key, ButtonText = "HelloChat Rooms Page", icon = app.icon, });		
				
			commandName = "Profile.HelloChat.Actions";
			command = System.App.Commands.AddNamedCommand(
				{name = commandName,app_key = app.app_key, ButtonText = "Actions Page", icon = app.icon, });	
				
			commandName = "Profile.HelloChat.CreateRoom";
			command = System.App.Commands.AddNamedCommand(
				{name = commandName,app_key = app.app_key, ButtonText = "Create Room Page", icon = app.icon, });
			
			commandName = "Profile.HelloChat.MyIncome";
			command = System.App.Commands.AddNamedCommand(
				{name = commandName,app_key = app.app_key, ButtonText = "My income page", icon = app.icon, });	
				
			commandName = "Profile.HelloChat.ShowAssetBag";	
			command = Map3DSystem.App.Commands.AddNamedCommand(
				{name = commandName,app_key = app.app_key, icon = app.icon, });	
			
			commandName = "Profile.HelloChat.ShowBCSBag";	
			command = Map3DSystem.App.Commands.AddNamedCommand(
				{name = commandName,app_key = app.app_key, icon = app.icon, });		
				
			commandName = "Profile.HelloChat.AddSelectionToAssetBag";	
			command = Map3DSystem.App.Commands.AddNamedCommand(
				{name = commandName, app_key = app.app_key, icon = app.icon, });		
			
			commandName = "Profile.HelloChat.SysCommandLine";	
			command = Map3DSystem.App.Commands.AddNamedCommand(
				{name = commandName, app_key = app.app_key, icon = app.icon, });		
			
			commandName = "Profile.HelloChat.DoSkill";
			command = Map3DSystem.App.Commands.AddNamedCommand(
				{name = commandName, app_key = app.app_key, icon = app.icon, });			
				
			commandName = "Profile.HelloChat.EnterChat";
			command = Map3DSystem.App.Commands.AddNamedCommand(
				{name = commandName, app_key = app.app_key, icon = app.icon, });				
			
			commandName = "Profile.HelloChat.Task";
			command = Map3DSystem.App.Commands.AddNamedCommand(
				{name = commandName, app_key = app.app_key, icon = app.icon, });					
		...
		-- e.g. 
		MyCompany.HelloChat.app = app; -- keep a reference
...
function MyCompany.HelloChat.OnDisconnection(app, disconnectMode)
	if(disconnectMode == System.App.DisconnectMode.UserClosed or disconnectMode == System.App.DisconnectMode.WorldClosed)then
		...
		local command = System.App.Commands.GetCommand("Profile.HelloChat.Login");
		if(command == nil) then
			command:Delete();
		end
	end
...
function MyCompany.HelloChat.OnQueryStatus(app, commandName, statusWanted)
...
function MyCompany.HelloChat.OnExec(app, commandName, params)
	if(commandName == "Profile.HelloChat.Login") then
		...
		System.App.Commands.Call("File.MCMLWindowFrame", {
			url=System.localserver.UrlHelper.BuildURLQuery("script/apps/HelloChat/Desktop/LoginPage.html", {cmdredirect=cmdredirect}), 
			name="HelloLogin.Wnd", 
			app_key=app.app_key, 
			text = title or "登陆窗口",
	...
	elseif(commandName == "Profile.HelloChat.Register") then
		-- register a new window
		System.App.Commands.Call("File.MCMLWindowFrame", {
			url="script/apps/HelloChat/Registration/HelloChatReg.html", 
			name="HelloReg.Wnd", 
			app_key=app.app_key, 
			text = "注册新用户",
			icon = "Texture/3DMapSystem/common/lock.png",
	...
	elseif(commandName == "Profile.HelloChat.CreateRoom") then
		-- register a new window
		System.App.Commands.Call("File.MCMLWindowFrame", {
			url="script/apps/HelloChat/Registration/HelloChatRegRoom.html", 
			name="HelloReg.Wnd", 
			app_key=app.app_key, 
			text = "申请我的社区",
			icon = "Texture/3DMapSystem/common/lock.png",
	...
	elseif(commandName == "Profile.HelloChat.Task") then	
		-- register a new window
		System.App.Commands.Call("File.MCMLWindowFrame", {
			url="script/apps/HelloChat/Task/HelloTask.html", 
			name="HelloTask.Wnd", 
			app_key=app.app_key, 
			text = "任务",
	...
	elseif(commandName == "Profile.HelloChat.SysCommandLine") then		
	...
	elseif(commandName == "File.EnterHelloWorld") then
		...
		params.res = res;
		if(res==true) then
			-- switch to HelloChat app_main desktop and make it default.
	...
	elseif(commandName == "Profile.HelloChat.EnterChat") then
		-- TODO: user pressed enter key to chat. 
		if(commonlib.getfield("MyCompany.HelloChat.ChatWnd.Show")) then
			MyCompany.HelloChat.ChatWnd.Show(true);
		end
		
	elseif(commandName == "Profile.HelloChat.DoSkill") then	
	...
	elseif(commandName == "Profile.HelloChat.ShowBCSBag") then	
		-- TODO: 
	...
	elseif(commandName == "Profile.HelloChat.ShowAssetBag") then
		-- show the asset bag. 
		if(type(params) == "string") then 
			params = {url=params};
		elseif(type(params) ~= "table") then 
	...
	elseif(commandName == "Profile.HelloChat.AddSelectionToAssetBag") then	
	...
	elseif(System.UI.AppDesktop.CheckUser(commandName)) then	
		...
		if(commandName == "Profile.HelloChat.HomePage") then
			System.App.Commands.Call("File.MCMLBrowser", {url="script/apps/HelloChat/Desktop/LoggedInHomePage.html", name="HelloPage", title="我的首页", DisplayNavBar = true});
		elseif(commandName == "Profile.HelloChat.Rooms") then
			System.App.Commands.Call("File.MCMLBrowser", {url="script/apps/HelloChat/Desktop/RoomsPage.html", name="HelloPage", title="邻居聊天室", DisplayNavBar = true});
		elseif(commandName == "Profile.HelloChat.Actions") then
			System.App.Commands.Call("File.MCMLBrowser", {url="script/apps/HelloChat/Desktop/ActionsPage.html", name="HelloPage", title="动作", DisplayNavBar = true});
		elseif(commandName == "Profile.HelloChat.MyIncome") then	
			System.App.Commands.Call("File.MCMLBrowser", {url="script/apps/HelloChat/Desktop/MyIncome.html", name="HelloPage", title="我的收益", DisplayNavBar = true});
		end
	elseif(app:IsHomepageCommand(commandName)) then
		MyCompany.HelloChat.GotoHomepage();
	elseif(app:IsNavigationCommand(commandName)) then
		MyCompany.HelloChat.Navigate();
	elseif(app:IsQuickActionCommand(commandName)) then	
		MyCompany.HelloChat.DoQuickAction();
	end
end

-- Change and render the 3D world with mcml data that is usually retrieved from the current user's profile page for this application. 
function MyCompany.HelloChat.OnRenderBox(mcmlData)
end


-- called when the user wants to nagivate to the 3D world location relavent to this application
function MyCompany.HelloChat.Navigate()
end

-- called when user clicks to check out the homepage of this application. Homepage usually includes:
-- developer info, support, developer worlds information, app global news, app updates, all community user rating, active users, trade, currency transfer, etc. 
function MyCompany.HelloChat.GotoHomepage()
end

-- called when user clicks the quick action for this application. 
function MyCompany.HelloChat.DoQuickAction()
end
...
function MyCompany.HelloChat.OnDeactivateDesktop()
end

-- user clicks to register
function MyCompany.HelloChat.OnUserRegister(btnName, values, bindingContext)
...
-----------------------------------------------------
function MyCompany.HelloChat.MSGProc(window, msg)
	----------------------------------------------------
	-- application plug-in messages here
	----------------------------------------------------
	if(msg.type == System.App.MSGTYPE.APP_CONNECTION) then	
		-- Receives notification that the Add-in is being loaded.
		MyCompany.HelloChat.OnConnection(msg.app, msg.connectMode);
		
	elseif(msg.type == System.App.MSGTYPE.APP_DISCONNECTION) then	
		-- Receives notification that the Add-in is being unloaded.
		MyCompany.HelloChat.OnDisconnection(msg.app, msg.disconnectMode);

	elseif(msg.type == System.App.MSGTYPE.APP_QUERY_STATUS) then
		-- This is called when the command's availability is updated. 
		-- NOTE: this function returns a result. 
		msg.status = MyCompany.HelloChat.OnQueryStatus(msg.app, msg.commandName, msg.statusWanted);
		
	elseif(msg.type == System.App.MSGTYPE.APP_EXEC) then
		-- This is called when the command is invoked.
		MyCompany.HelloChat.OnExec(msg.app, msg.commandName, msg.params);
				
	elseif(msg.type == System.App.MSGTYPE.APP_RENDER_BOX) then	
		-- Change and render the 3D world with mcml data that is usually retrieved from the current user's profile page for this application. 
		MyCompany.HelloChat.OnRenderBox(msg.mcml);
	...
	elseif(msg.type == System.App.MSGTYPE.APP_ACTIVATE_DESKTOP) then
		MyCompany.HelloChat.OnActivateDesktop();
		
	elseif(msg.type == System.App.MSGTYPE.APP_DEACTIVATE_DESKTOP) then
		MyCompany.HelloChat.OnDeactivateDesktop();
