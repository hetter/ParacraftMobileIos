--[[
Title: Sample app for Paraworld
Author(s): LiXizhi
Date: 2008/1/5
Desc: 
db registration insert script
INSERT INTO apps VALUES (NULL, 'Sample_GUID', 'Sample', '1.0.0', 'http://www.paraengine.com/apps/Sample_v1.zip', 'YourCompany', 'enUS', 'script/apps/sample/IP.xml', '', 'script/apps/sample/app_main.lua', 'MyCompany.Apps.Sample.MSGProc', 1);

use the lib:
------------------------------------------------------------
NPL.load("(gl)script/apps/sample/app_main.lua");
------------------------------------------------------------
]]
...
-- create class
commonlib.setfield("MyCompany.Apps.Sample", {});
...
function MyCompany.Apps.Sample.OnConnection(app, connectMode)
	if(connectMode == Map3DSystem.App.ConnectMode.UI_Setup) then
		...
		-- e.g. Create a sample command link in the main menu 
		local commandName = "Profile.Sample";
		local command = Map3DSystem.App.Commands.GetCommand(commandName);
		if(command == nil) then
			command = Map3DSystem.App.Commands.AddNamedCommand(
				{name = commandName,app_key = app.app_key, ButtonText = "From Sample UI Setup", icon = app.icon, });
		...
		if(ParaEngine.GetLocale() == "zhCN") then
			app.HomeButtonText = "Sample in Chinese";
		else
			app.HomeButtonText = "Sample in English";
		end
	end
end

-- Receives notification that the Add-in is being unloaded.
function MyCompany.Apps.Sample.OnDisconnection(app, disconnectMode)
	if(disconnectMode == Map3DSystem.App.DisconnectMode.UserClosed or disconnectMode == Map3DSystem.App.DisconnectMode.WorldClosed)then
		...
		local command = Map3DSystem.App.Commands.GetCommand("Profile.Sample");
		if(command == nil) then
			command:Delete();
		end
	end
...
function MyCompany.Apps.Sample.OnQueryStatus(app, commandName, statusWanted)
	if(statusWanted == Map3DSystem.App.CommandStatusWanted) then
		...
		if(commandName == "Profile.Sample") then
...
-- @param commandName: The name of the command to determine state for. Usually in the string format "Category.SubCate.Name".
function MyCompany.Apps.Sample.OnExec(app, commandName, params)
	if(commandName == "Profile.Sample") then
		-- TODO: actual code of processing the command goes here. 
		-- e.g.
		_guihelper.MessageBox("Sample application executed. ");
		
	elseif(app:IsHomepageCommand(commandName)) then
		MyCompany.Apps.Sample.GotoHomepage();
	elseif(app:IsNavigationCommand(commandName)) then
		MyCompany.Apps.Sample.Navigate();
	elseif(app:IsQuickActionCommand(commandName)) then	
		MyCompany.Apps.Sample.DoQuickAction();
	end
end

-- Change and render the 3D world with mcml data that is usually retrieved from the current user's profile page for this application. 
function MyCompany.Apps.Sample.OnRenderBox(mcmlData)
end


-- called when the user wants to nagivate to the 3D world location relavent to this application
function MyCompany.Apps.Sample.Navigate()
end

-- called when user clicks to check out the homepage of this application. Homepage usually includes:
-- developer info, support, developer worlds information, app global news, app updates, all community user rating, active users, trade, currency transfer, etc. 
function MyCompany.Apps.Sample.GotoHomepage()
end

-- called when user clicks the quick action for this application. 
function MyCompany.Apps.Sample.DoQuickAction()
end
