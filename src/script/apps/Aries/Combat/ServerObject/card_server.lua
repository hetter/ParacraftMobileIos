...
function Card:CreateCardTemplate(datafile)
	...
	LOG.std(nil, "info", "card_server", "server loaded card template file %s", datafile);
...
function Card.UseCard(key, card_seq, arena, caster_data, target_data, sequence, original_key)
	...
	if(template.type == "AreaAttack" or template.type == "ArenaAttack" or template.type == "AreaHeal" or 
		...
		if(not target) then
			...
			spell_str = format("pass:%d,%d,%s,%d", arena:GetID(), arrow_position, tostring(caster_data.isMob), caster_data.id);
	...
	for _, dot in ipairs(dot_damages) do
		...
		if(dot_damage > 0) then
			...
			spell_str = format("dot:%d,%d,%s,%d,%s,%s%d#%s", arena:GetID(), arrow_position, tostring(caster_data.isMob), caster_data.id, dot_damage_school, dmg_mark, dot_damage, caster:GetWardsValue());
			table_insert(sequence, spell_str);
		...
		elseif(dot_damage < 0) then
			...
			for _, unit in ipairs(siblins) do
				...
				if(not unit:IsAlive()) then
					...
					local spell_str = "";
					spell_str = format("dead:%d,%d,%s,%d", arena:GetID(), unit:GetArrowPosition_id(), tostring(unit:IsMob()), unit:GetID());
					table_insert(extra_play_seq, spell_str);
					sequence.duration = sequence.duration + Card.GetSpellDuration_from_card_spellname("Dead");
					-- mark battlefield death
				...
				ismob_id_damage = format("(%s,%d,%d,%s%d#%s#%s#%s#%s#%s#%s)", tostring(unit:IsMob()), unit:GetID(), unit:GetArrowPosition_id(), dmg_mark, this_dot_damage, unit:GetCharmsValue(), unit:GetCharmsValue(), last_target_wards, unit:GetWardsValue(), last_target_overtimes, unit:GetOverTimeValue());
			...
			-- append spell play
			local spell_str = "";
			spell_str = format("%s:%s,%d,%d,%s,%d+%s+%s+%s", "DoT_Fire_Splash", "DoT_Fire_Splash", arena:GetID(), arrow_position, tostring(caster_data.isMob), caster_data.id, last_caster_charms, caster:GetCharmsValue(), ismob_id_damage_blocks);
			table_insert(sequence, spell_str);
	...
	for _, hot in ipairs(hot_heals) do
		...
		local spell_str = "";
		local arrow_position = caster:GetArrowPosition_id();
		spell_str = format("hot:%d,%d,%s,%d,%d#%s", arena:GetID(), arrow_position, tostring(caster_data.isMob), caster_data.id, hot_heal, caster:GetWardsValue());
		table_insert(sequence, spell_str);
	...
	if(template.type == "PickPet" and caster_data.isMob == false) then
		...
		if(bSuccess and gsid) then
			...
			local arrow_position = caster:GetArrowPosition_id();
			spell_str = format("pickpet:%d,%d,%s,%d,%d,%s,%d,%d,%s,%d,%d", 
	...
	if(template.type == "CatchPet" and caster_data.isMob == false and target_data.isMob == true) then
		...
		if(bSuccess) then
			...
			local spell_school = caster:GetPhase();
			local arrow_position = caster:GetArrowPosition_id();
			spell_str = format("balance_catchpet_success:%d,%d,%s,%d,%d,%s,%d,%d,%s", 
						arena:GetID(), arrow_position, 
						tostring(caster_data.isMob), caster_data.id, caster:GetArrowPosition_id(), 
			...
			if(System.options.version ~= "teen") then
				...
				spell_str = format("dead:%d,%d,%s,%d", arena:GetID(), arrow_position, tostring(target_data.isMob), target_data.id);
				table_insert(sequence, spell_str);
				sequence.duration = sequence.duration + Card.GetSpellDuration_from_card_spellname("Dead");
			end
			-- suiside pet
			...
			local spell_school = caster:GetPhase();
			local arrow_position = caster:GetArrowPosition_id();
			spell_str = format("balance_catchpet_failed:%d,%d,%s,%d,%d,%s,%d,%d,%s", 
						arena:GetID(), arrow_position, 
						tostring(caster_data.isMob), caster_data.id, caster:GetArrowPosition_id(), 
						tostring(target_data.isMob), target:GetID(), target:GetArrowPosition_id(), 
						spell_school);
	...
	if(template.type == "Enrage" and caster_data.isMob == false and target_data.isMob == true) then
		...
		if(bSuccess) then
			...
			local arrow_position = caster:GetArrowPosition_id();
			spell_str = format("%s:%d,%d,%s,%d,%d,%s,%d,%d,%s", 
	...
	if(r > accuracy or not spell_name) then
		...
		local spell_school = template.spell_school;
		local arrow_position = caster:GetArrowPosition_id();
		spell_str = format("fizzle:%d,%d,%s,%d,%d,%s,%d,%d,%s", 
					arena:GetID(), arrow_position, 
					tostring(caster_data.isMob), caster_data.id, caster:GetArrowPosition_id(), 
	...
	if(caster:IsMob() == false) then
		if(card_seq and card_seq >= 10000) then
			...
			if(gsid) then
				local spell_str = "";
				local arrow_position = caster:GetArrowPosition_id();
				spell_str = format("use_petcard:%d,%d,%s,%d,%s", arena:GetID(), arrow_position, tostring(caster_data.isMob), caster_data.id, tostring(gsid));
				table_insert(sequence, spell_str);
			...
			local spell_str = "";
			local arrow_position = caster:GetArrowPosition_id();
			spell_str = format("next_spell_is_from_petcard:%d,%d,%s,%d", arena:GetID(), arrow_position, tostring(caster_data.isMob), caster_data.id);
			table_insert(sequence, spell_str);
		end
	end
	...
	if(template.type == "SingleAttackWithMultipleDamage") then
		...
		for i = 1,attack_times do
			if(target:IsAlive()) then
				...
				local spell_str = "";
				if(i == 1) then
					spell_str = format("%s:%s,%d,%d,%s,%d,%d,%s,%d,%d,%s,%d#%s#%s#%s#%s#%s#%s#%s#%s", spell_name, key, arena:GetID(), caster_arrow_position, 
							tostring(caster_data.isMob), caster_data.id, caster:GetArrowPosition_id(), 
							tostring(target_data.isMob), target_data.id, target:GetArrowPosition_id(), 
					...
					spell_str = format("dot:%d,%d,%s,%d,%s,%s%d#%s", arena:GetID(), target_arrow_position, tostring(target_data.isMob), target_data.id, damage_school, dmg_mark, damage, target:GetWardsValue());
					table_insert(sequence, spell_str);
	...
	elseif(template.type == "SingleAttack" or template.type == "SingleAttackWithDOT" or 
		...
		-- append spell play
		local spell_str = "";
		spell_str = format("%s:%s,%d,%d,%s,%d,%d,%s,%d,%d,%s,%d#%s#%s#%s#%s#%s#%s#%s#%s", spell_name, key, arena:GetID(), arrow_position, 
					tostring(caster_data.isMob), caster_data.id, caster:GetArrowPosition_id(), 
					tostring(target_data.isMob), target_data.id, target:GetArrowPosition_id(), 
					(dmg_mark or "")..damage, secondary_param, 
					last_caster_charms, caster:GetCharmsValue(), 
	...
	elseif(template.type == "SingleHeal" or template.type == "SingleHealWithHOT" or template.type == "SingleHealWithImmolate" or template.type == "SingleHealWithCleanse") then
		...
		-- append spell play
		local spell_str = "";
		spell_str = format("%s:%s,%d,%d,%s,%d,%d,%s,%d,%d,%d,%d#%s#%s#%s#%s#%s#%s#%s#%s", spell_name, key, arena:GetID(), arrow_position, 
					tostring(caster_data.isMob), caster_data.id, caster:GetArrowPosition_id(), 
					tostring(target_data.isMob), target_data.id, target:GetArrowPosition_id(), 
	...
	elseif(template.type == "AreaAttack" or template.type == "ArenaAttack" or template.type == "AreaDOTAttack" or template.type == "AreaAttackWithDOT" or template.type == "AreaAttackWithStun" or template.type == "AreaAttackWithImmolate" or template.type == "AreaAttackWithExtraThreat") then
		...
		for i = 1, #targets do
			...
			local ismob_id_damage = "";
			ismob_id_damage = format("(%s,%d,%d,%s#%s#%s#%s#%s#%s#%s)", 
					tostring(each_target:IsMob()), each_target:GetID(), each_target:GetArrowPosition_id(), 
		...
		-- append spell play
		local spell_str = "";
		spell_str = format("%s:%s,%d,%d,%s,%d+%s+%s+%s", spell_name, key, arena:GetID(), arrow_position, tostring(caster_data.isMob), caster_data.id, last_caster_charms, caster:GetCharmsValue(), ismob_id_damage_blocks);
		table_insert(sequence, spell_str);
		sequence.duration = sequence.duration + Card.GetSpellDuration_from_card_spellname(spell_name);
		-- mark battlefield attack points
		local _, mapping;
	...
	elseif(template.type == "AreaPowerPipBoost") then
		...
		for i = 1, #targets do
			...
			local ismob_id_heal = "";
			ismob_id_heal = format("(%s,%d,%d,%d#%s#%s#%s#%s#%s#%s)", tostring(each_target:IsMob()), each_target:GetID(), each_target:GetArrowPosition_id(), heal, each_target:GetCharmsValue(), each_target:GetCharmsValue(), last_target_wards, each_target:GetWardsValue(), last_target_overtimes, each_target:GetOverTimeValue());
		...
		-- append spell play
		local spell_str = "";
		spell_str = format("%s:%s,%d,%d,%s,%d+%s+%s+%s", spell_name, key, arena:GetID(), arrow_position, tostring(caster_data.isMob), caster_data.id, last_caster_charms, caster:GetCharmsValue(), ismob_id_heal_blocks);
		table_insert(sequence, spell_str);
		sequence.duration = sequence.duration + Card.GetSpellDuration_from_card_spellname(spell_name);
		-- append extra play sequence
		local _, seq;
	...
	elseif(template.type == "AreaCleanse") then
		...
		for i = 1, #targets do
			...
			local ismob_id_heal = "";
			ismob_id_heal = format("(%s,%d,%d,%d#%s#%s#%s#%s#%s#%s)", tostring(each_target:IsMob()), each_target:GetID(), each_target:GetArrowPosition_id(), 0, each_target:GetCharmsValue(), each_target:GetCharmsValue(), last_target_wards, each_target:GetWardsValue(), last_target_overtimes, each_target:GetOverTimeValue());
			ismob_id_heal_blocks = ismob_id_heal_blocks..ismob_id_heal;
		end
	...
	elseif(template.type == "AreaStun") then
		...
		for i = 1, #targets do
			...
			local ismob_id = "";
			ismob_id = format("(%s,%d,%d,0#%s#%s#%s#%s#%s#%s)", tostring(each_target:IsMob()), each_target:GetID(), each_target:GetArrowPosition_id(), 
		...
		-- append spell play
		local spell_str = "";
		spell_str = format("%s:%s,%d,%d,%s,%d+%s+%s+%s", spell_name, key, arena:GetID(), arrow_position, tostring(caster_data.isMob), caster_data.id, caster:GetCharmsValue(), caster:GetCharmsValue(), ismob_id_blocks);
		table_insert(sequence, spell_str);
		sequence.duration = sequence.duration + Card.GetSpellDuration_from_card_spellname(spell_name);
		-- update arena
		local key, value = arena:GetKey_normal_update(), arena:GetValue_normal_update();
	...
	elseif(template.type == "AreaCharm") then
		...
		for i = 1, #targets do
			...
			last_target_charms = "0,"..last_target_charms;
			local ismob_id = "";
			ismob_id = format("(%s,%d,%d,0#%s#%s#%s#%s#%s#%s)", tostring(each_target:IsMob()), each_target:GetID(), each_target:GetArrowPosition_id(), last_target_charms, each_target:GetCharmsValue(), each_target:GetWardsValue(), each_target:GetWardsValue(), each_target:GetOverTimeValue(), each_target:GetOverTimeValue());
			ismob_id_blocks = ismob_id_blocks..ismob_id;
		end
...
function Card.UseCard_post(key, arena, caster_data, target_data, sequence, original_key)
	...
	for i = 1, 8 do
		...
		if(unit and not unit:IsAlive() and unit.bWithGuardian and unit:IsCombatActive()) then
			...
			local spell_str = "";
			local arrow_position = unit:GetArrowPosition_id();
			spell_str = format("revive:%d,%d,%s,%d", arena:GetID(), arrow_position, tostring(unit:IsMob()), unit:GetID());
			table_insert(sequence, spell_str);
			...
			local spell_str = "";
			local arrow_position = unit:GetArrowPosition_id();
			spell_str = format("revive_post:%d,%d,%s,%d", arena:GetID(), arrow_position, tostring(unit:IsMob()), unit:GetID());
			table_insert(sequence, spell_str);
