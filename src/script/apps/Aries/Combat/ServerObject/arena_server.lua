...
function Arena:OnFrameMove(curTime)
	...
	if(isActive == false) then
		...
		elseif(self.mode == "free_pvp") then
			...
			self:MarkWinningAndLosingRankingPoint();
...
-- check the ranking point on every arena frame move
-- update the player winning and losing point change after all player data is refreshed
function Arena:MarkWinningAndLosingRankingPoint()
...
function Arena:FinishCombat()
	...
	for each_nid, used_card_count in pairs(used_card_counts) do
		...
		if(not player:IsMinion()) then -- not minion
			local original_exp = 0;
			if(max_used_card_count and max_used_card_count > 0) then
				original_exp = math.ceil(this_exp * (1 - math_min((max_used_card_count - used_card_count) * 0.1, 0.5)));
			end
	...
	if(player) then
		...
		if(arena_id) then
			...
			if(arena and arena.seq == seq) then
				...
				if(string.find(string.lower(card_key), "areaattack")) then
					-- continue
					-- -1 stands for all mobs for areaattack
					isMob = hostile_isfarside;
					id = -1;
				elseif(string.find(string.lower(card_key), "areataunt")) then
					-- continue
					-- -1 stands for all mobs for areataunt
					isMob = hostile_isfarside;
					id = -1;
				elseif(string.find(string.lower(card_key), "arenaattack")) then
					-- continue
					-- -1 stands for all mobs for areaattack
					isMob = hostile_isfarside;
					id = -1;
				elseif(string.find(string.lower(card_key), "areadotattack")) then
					-- continue
					-- -1 stands for all mobs for areadotattack
					isMob = hostile_isfarside;
					id = -1;
				elseif(string.find(string.lower(card_key), "areasingleattack")) then
					-- continue
					-- -1 stands for all mobs for areasingleattack
					isMob = hostile_isfarside;
					id = -1;
				elseif(string.find(string.lower(card_key), "areastunabsorb")) then
					-- continue
					-- -1 stands for all players for areastunabsorb
					isMob = friendly_isfarside;
					id = -1;
				elseif(string.find(string.lower(card_key), "areastun")) then
					-- continue
					-- -1 stands for all mobs for areastun
					isMob = hostile_isfarside;
					id = -1;
				elseif(string.find(string.lower(card_key), "areadamageweakness")) then
					-- continue
					-- -1 stands for all mobs for areadamageweakness
					isMob = hostile_isfarside;
					id = -1;
				elseif(string.find(string.lower(card_key), "areadamageshield")) then
					-- continue
					-- -1 stands for all players for areastunabsorb
					isMob = friendly_isfarside;
					id = -1;
				elseif(string.find(string.lower(card_key), "areaaccuracyweakness")) then
					-- continue
					-- -1 stands for all mobs for areaaccuracyweakness
					isMob = hostile_isfarside;
					id = -1;
				elseif(string.find(string.lower(card_key), "areadamagetrap")) then
					-- continue
					-- -1 stands for all mobs for areadamagetrap
					isMob = hostile_isfarside;
					id = -1;
				elseif(string.find(string.lower(card_key), "areaaccuracyblade")) then
					-- continue
					-- -1 stands for all players for areaaccuracyblade
					isMob = friendly_isfarside;
					id = -1;
				elseif(string.find(string.lower(card_key), "areaheal")) then
					-- continue
					-- -1 stands for all players for areaheal
					isMob = friendly_isfarside;
					id = -1;
				elseif(string.find(string.lower(card_key), "areacleanse")) then
					-- continue
					-- -1 stands for all players for areaheal
					isMob = friendly_isfarside;
					id = -1;
				elseif(string.find(string.lower(card_key), "areapowerpipboost")) then
					-- continue
					-- -1 stands for all players for areaheal
					isMob = friendly_isfarside;
					id = -1;
				...
				if(string.find(string.lower(card_key), "miniaura")) then
