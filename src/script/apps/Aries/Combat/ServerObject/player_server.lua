...
function Player:GetValue_normal_update()
	local value = "";
	value = format("%s,%d,%d,%s,%d,%d,%d,%d,%d,%d#%s#%s#%s#%s", 
		tostring(self:IsMob()),
		self:GetNID(),
...
function Player:GetOverTimeValue()
	...
	if(self:GetNID() < 0 and self.turns_played and self.turns_played >= 2) then
		-- immune to kickpet
...
function Player:ActiveCombat()
	if(self.bCombatActive ~= true and not self.bResyncCombatItems and self.nid and self.nid > 0) then
