...
function CommandManager:GetCmdList(cmds_str, line_reg_exp)
	if(cmds_str) then
		...
		local out = {};
		commonlib.echo("!!!!!!!!!!!!!!!!!!!!!!!!!!!! ......... CommandManager:GetCmdList:" .. cmds_str);
		for comment, cmd in string.gmatch(cmds_str, line_reg_exp) do
			if(comment == "") then
				commonlib.echo("!!!!!!!!!!!!!!!!!!!!!!!!!............... CommandManager:GetCmdList:" .. cmd);
				out[#out+1] = cmd
			--else
				--out[#out+1] = "";
			end
...
function CommandManager:RunCmdSegment(cmd_list, fromLine, toLine, variables, fromEntity)
	...
	commonlib.echo("!!!!!!!!!!!!!!!!!!!!!!!!!............... CommandManager:RunCmdSegment:line_count" .. line_count);
	
	while line_index <= line_count do
		commonlib.echo("!!!!!!!!!!!!!!!!!!!!!!!!!............... CommandManager:RunCmdSegment:line_index" .. line_index);
		
		local cmd = cmd_list[line_index];
		if(cmd~="") then
			last_result, goto_label = self:RunWithVariables(variables, cmd, fromEntity);
			if( last_result == false) then
				commonlib.echo("!!!!!!!!!!!!!!!!!!!!!!!!!............... CommandManager:RunCmdSegment:last_result == false:" .. cmd);
				if(goto_label) then
