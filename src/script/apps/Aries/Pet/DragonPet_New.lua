...
function DragonPet_New:Update()
	...
	--如果升级了
	if(not oldBean or ( oldBean and currentBean and oldBean.level < currentBean.level) )then
		--升级语言
	...
	--魔法星升级
	if(not oldBean or ( oldBean and currentBean and oldBean.mlel < currentBean.mlel) )then
	...
	--魔法星复活 energy 从0变为非0
	if(not oldBean or ( oldBean and currentBean and oldBean.energy == 0 and  currentBean.energy > 0) )then
	...
	-- combat level update
	if(self:GetNID() == ProfileManager.GetNID() and ( oldBean and currentBean and oldBean.combatlel < currentBean.combatlel) )then
