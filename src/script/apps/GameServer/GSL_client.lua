...
-- @param nid: should be game server nid.
...
function client:LoginServer(nid, ws_id, worldpath, params)
	...
	if(self.gatewayProxy.nid~=nid or self.gatewayProxy.nid.worldserver_id~=ws_id) then
		self:LogoutServer();
...
local function activate()
	...
	if(game_client and game_client.bCanReceive) then
		...
		LOG.std("", "warning", "GSL", "warning: we ignored a message from nid %s", msg.nid);
	end
end
NPL.this(activate);
