...
--			end
	...
	local function HandleCallback(id, API_key, data)
		commonlib.echo("===========HandleCallback API_key:" .. API_key);
	...
	local function activate(self, msg, id, callbackFunc, callbackParams)
		...
		if(res == nil) then
			...
			if(res==true) then
				...
				commonlib.echo("===========set HasRegisterHandleCallBacks:" .. API_key.."_Response");
...
--
local function setRegisterHandleCallBacks(key, func)
	if not HasRegisterHandleCallBacks[key] then
		HasRegisterHandleCallBacks[key] = func;
	end
end
...
local ProtocolResponseMap = {};
local function callLuaFunc_Response(key, value)
	if not LocalBridgePB_pb then
		NPL.load("(gl)script/mobile/NetWork/LocalBridgePB_pb.lua")
	end
	
	local rsp = LocalBridgePB_pb["callLuaFunc_Response"]();
	rsp:ParseFromString(value)
	-- create a raw data table as returned message
	local raw_rsp = ParseRawDataFromProtoMsg(rsp);
	
	commonlib.echo("===========callLuaFunc_Response protocolId:" .. raw_rsp.protocolId);
	commonlib.echo("===========callLuaFunc_Response data:" .. raw_rsp.data);
	
	local protocolId = raw_rsp.protocolId;
	local data = raw_rsp.data;
	if ProtocolResponseMap[protocolId] then
		ProtocolResponseMap[protocolId](protocolId, data);
	end
end

setRegisterHandleCallBacks("callLuaFunc_Response", callLuaFunc_Response);

function LocalService.RegisterProtocolCallBacks(protocolId, callBackFunc)
	ProtocolResponseMap[protocolId] = callBackFunc;
end
...
local function activate()
	...
	commonlib.echo("===========activate:" .. key);
	
	--if key == "my_test_Response" then
	--	commonlib.echo("===========my_test_Response:"..tostring(value));
	--else
