...
PluginLoader:Property({"PluginFolder", "", auto=true, desc="default plugin folder"});
...
function PluginLoader.SearchFiles(output, rootfolder,nMaxFilesNum, filter)
	if(rootfolder == nil) then return; end
	if(filter == nil) then filter = "*." end
	
	output = output or {};
	local sInitDir2 = ParaIO.GetCurDirectory(0)..rootfolder.."/";
	-- mobile version must not append ParaIO.GetCurDirectory(0) to InitDir.
	local sInitDir = rootfolder.."/";

	log("[PluginLoader] sInitDir2....:" .. tostring(sInitDir2) .. "\n");
	log("[PluginLoader] sInitDir....:" .. tostring(sInitDir) .. "\n");
...
function PluginLoader:SearchAllModules()
	...
	local folderPath = "Mod/";
	log("[PluginLoader] folderPath....:" .. tostring(folderPath) .. "\n");
	--local output = commonlib.Files.Find(nil, folderPath, 0, 50000, "*.");
	local output = PluginLoader.SearchFiles(nil, folderPath, 50000);
	if(output and #output>0) then
		for _, item in ipairs(output) do
			local filename = format("%s/main.lua", item.filename);
			if(ParaIO.DoesFileExist(filename, false)) then
				log("yes ParaIO.DoesFileExist(filename, false)" .. tostring(filename) .. "\n");
			else
				log("no ParaIO.DoesFileExist(filename, false)" .. tostring(filename) .. "\n");
			--else
...
function PluginLoader:DoesPluginExist(modname)
	...
	log("PluginLoader:DoesPluginExist:" .. tostring(modname));
